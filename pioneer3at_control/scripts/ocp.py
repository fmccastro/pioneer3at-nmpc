...
class GPR(Callback):
  ...
  def eval(self, arg):
    ...
    [pred,_] = model.predict_y(np.array(arg[0]).T) #evaluate GPR
...
model = gpflow.models.GPR(data = ( data, value ), kernel = gpflow.kernels.Constant(nd) + gpflow.kernels.Linear(nd) + gpflow.kernels.White(nd) + gpflow.kernels.RBF(nd), mean_function = None )

#train parameters
gpflow.optimizers.Scipy().minimize( model.training_loss, model.trainable_variables, options = dict( maxiter = 100 ) )

gpr = GPR('GPR', {"enable_fd":True})        #enable finite differences 
...
plt.savefig('ocp.png', bbox_inches='tight')
